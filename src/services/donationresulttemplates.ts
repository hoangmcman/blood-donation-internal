...
export const getDonationResultTemplates = async (
	page: number = 1,
	limit: number = 10
): Promise<DonationResultTemplateResponse> => {
...
export const getDonationResultTemplateById = async (
	id: string
): Promise<ApiResponse<DonationResultTemplate>> => {
	const response = await api.get<ApiResponse<DonationResultTemplate>>(
		`/donation-result-templates/${id}`
	);
	return response.data;
};
...
export const createDonationResultTemplate = async (
	payload: CreateDonationResultTemplatePayload
): Promise<ApiResponse<DonationResultTemplate>> => {
	const response = await api.post<ApiResponse<DonationResultTemplate>>(
		"/donation-result-templates",
		payload
	);
	return response.data;
};
...
export const updateDonationResultTemplate = async (
	id: string,
	payload: UpdateDonationResultTemplatePayload
): Promise<ApiResponse<DonationResultTemplate>> => {
	const response = await api.patch<ApiResponse<DonationResultTemplate>>(
		`/donation-result-templates/${id}`,
		payload
	);
	return response.data;
...
export const updateDonationResultTemplateItem = async (
	templateId: string,
	itemId: string,
	payload: Partial<Item>
): Promise<ApiResponse<Item>> => {
	const response = await api.patch<ApiResponse<Item>>(
		`/donation-result-templates/${templateId}/items/${itemId}`,
		payload
	);
	return response.data;
};
...
export const deleteDonationResultTemplateItem = async (
	templateId: string,
	itemId: string
...
export const getDonationResultTemplateOptions = async (
	itemId: string,
	page: number = 1,
	limit: number = 10
): Promise<ApiResponse<{ data: Option[]; meta: PaginationMeta }>> => {
	const response = await api.get<ApiResponse<{ data: Option[]; meta: PaginationMeta }>>(
		`/donation-result-templates/items/${itemId}/options`,
		{
			params: { page, limit },
		}
	);
...
export const useGetDonationResultTemplateOptions = (
	itemId: string,
	page: number = 1,
	limit: number = 10
) => {
...
export const getDonationResultTemplateOptionById = async (
	optionId: string
): Promise<ApiResponse<Option>> => {
	const response = await api.get<ApiResponse<Option>>(
		`/donation-result-templates/options/${optionId}`
	);
	return response.data;
};
...
export const createDonationResultTemplateOption = async (
	itemId: string,
	payload: { label: string }
): Promise<ApiResponse<Option>> => {
	const response = await api.post<ApiResponse<Option>>(
		`/donation-result-templates/items/${itemId}/options`,
		payload
	);
	return response.data;
	...
	return useMutation({
		...
		onSuccess: (_, variables) => {
			queryClient.invalidateQueries({
				queryKey: ["donationResultTemplateOptions", variables.itemId],
			});
		},
	});
};

export const deleteDonationResultTemplateOption = async (
	optionId: string
): Promise<ApiResponse<void>> => {
	const response = await api.delete<ApiResponse<void>>(
		`/donation-result-templates/options/${optionId}`
