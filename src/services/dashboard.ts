...
export const getBloodTypeDistribution = async (
	startDate: string,
	endDate: string
): Promise<ApiResponse<BloodTypeStats>> => {
	const response = await api.get<ApiResponse<BloodTypeStats>>(
		"/donation/stats/blood-type-distribution",
		{
			params: { startDate, endDate },
		}
	);
...
export const getDonorStats = async (
	startDate: string,
	endDate: string
): Promise<ApiResponse<DonorStats>> => {
