...
import { useUpdateDonationResultTemplate } from "../../services/donationresulttemplates"
import { type DonationResultTemplate } from "../../services/donationresulttemplates"
...
interface EditDonationResultTemplateDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  template: DonationResultTemplate
}

export function EditDonationResultTemplateDialog({ open, onOpenChange, template }: EditDonationResultTemplateDialogProps) {
  ...
  const updateMutation = useUpdateDonationResultTemplate()

  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      // Map items to include id and ensure options match Option interface
      const itemsWithIdAndOptions = values.items.map((item, idx) => {
        const originalItem = template.items?.[idx] || { id: uuidv4() };
        return {
          id: originalItem.id,
          type: item.type,
          label: item.label,
          ...
          options: item.options?.map(option => ({
            id: originalItem.options?.find(o => o.label === option.label)?.id || uuidv4(),
            label: option.label,
          })) || [],
        };
      });
      await updateMutation.mutateAsync({ id: template.id, payload: { ...values, items: itemsWithIdAndOptions } })
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Chỉnh sửa Mẫu Kết Quả Quyên Góp</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Button type="submit" className="w-full">Cập nhật mẫu</Button>
          </form>
        </Form>
      </DialogContent>
    </Dialog>
