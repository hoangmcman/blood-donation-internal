...
import { type Item, type UpdateDonationResultPayload } from "../../services/donations"
...
export default function UpdateDonationResultDialog({ open, onOpenChange, templateId, donationResultId }: UpdateDonationResultDialogProps) {
  ...
  React.useEffect(() => {
    if (data?.data) {
      const initialBloodTestResults = data.data.items.reduce((acc, item) => ({
        ...acc,
          ...
          <div className="space-y-4">
            ...
            {template.items.map((item: Item) => (
              <div key={item.id} className="space-y-2">
                <Label>{item.label} {item.isRequired && <span className="text-red-500">*</span>}</Label>
