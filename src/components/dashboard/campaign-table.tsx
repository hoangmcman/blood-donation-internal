...
import { useGetCampaignStats } from "@/services/dashboard"
...
export function CampaignTable() {
  ...
  const currentDate = new Date("2025-07-23T15:35:00+07:00")
  ...
  const { data: campaignStats, isLoading } = useGetCampaignStats(
    getDateRange(timeRange).startDate,
    getDateRange(timeRange).endDate
  )

  React.useEffect(() => {
    if (campaignStats?.data.campaigns) {
      setData(campaignStats.data.campaigns)
    }
  }, [campaignStats])
